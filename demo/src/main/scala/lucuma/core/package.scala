...
package svg {
  ...
  object implicits {
    ...
    implicit val pointCollection: RenderSvg[Point] =
      new RenderSvg[Point] {
        def toSvg(
          base:      Container,
          pp:        SvgPostProcessor,
          ...
          // Circle is a Container but the svg facade doesn't reflect that
          val point =
            base.circle().translate(scalingFn(a.getX), scalingFn(a.getY)).radius(1000)
          point.asInstanceOf[Container]
        }
      }
      ...
      new RenderSvg[Geometry] {
        def toSvg(
          ...
          a match {
            ...
            case p: Point              => p.toSvg(base, pp, scalingFn)
            case _                     =>
