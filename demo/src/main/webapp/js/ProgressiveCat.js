...
const ProgressiveCat = (function () {
  ...
  ProgressiveCat.readProperties = function (
    rootUrl,
    successCallback,
    errorCallback
  ) {
    ...
    $(xml)
      ...
      .each(function () {
        ...
        if (
          !instance.keyRa &&
          f.ucd &&
          (f.ucd.indexOf("pos.eq.ra") === 0 || f.ucd.indexOf("POS_EQ_RA") === 0)
        ) {
          if (f.name) {
        ...
        if (
          !instance.keyDec &&
          f.ucd &&
          (f.ucd.indexOf("pos.eq.dec") === 0 ||
            f.ucd.indexOf("POS_EQ_DEC") === 0)
        ) {
          if (f.name) {
  ...
  function getSources(instance, csv, fields) {
    ...
    for (var i = 2; i < lines.length; i++) {
      ...
      var ra, dec;
      if (
        Utils.isNumber(mesures[instance.keyRa]) &&
        Utils.isNumber(mesures[instance.keyDec])
      ) {
