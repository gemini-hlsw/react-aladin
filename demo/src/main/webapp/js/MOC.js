...
const MOC = (function () {
  ...
  MOC.prototype._addPix = function (order, ipix) {
    ...
    // 2. if LOWRES_MAXORDER < order <= HIGHRES_MAXORDER , degrade ipix for low res cells
      ...
      let degradedIpixOrder3 = Math.floor(
        degradedIpix * Math.pow(4, 3 - degradedOrder)
      );
      if (!(degradedOrder in this._lowResIndexOrder3[degradedIpixOrder3])) {
        this._lowResIndexOrder3[degradedIpixOrder3][degradedOrder] = [];
      }
      this._lowResIndexOrder3[degradedIpixOrder3][degradedOrder].push(
        degradedIpix
      );
    }
      ...
      try {
        ...
        if (this.hdus.length === 0) {
          if (self.proxyCalled !== true) {
            ...
            var proxiedURL =
              Aladin.JSONP_PROXY + "?url=" + encodeURIComponent(self.dataURL);
  ...
  MOC.prototype.draw = function (
    ...
    var mocCells =
      fov > MOC.PIVOT_FOV && this.adaptativeDisplay
  ...
  MOC.prototype._drawCells = function (
    ctx,
    mocCellsIdxOrder3,
    fov,
    projection,
    ...
    var potentialVisibleHpxCellsOrder3 = this.view.getVisiblePixList(
      3,
      CooFrameEnum.J2000
    );
  ...
  var getXYCorners = function (
    ...
    if (
      cornersXYView[0].vx >= width &&
      cornersXYView[1].vx >= width &&
      cornersXYView[2].vx >= width &&
      cornersXYView[3].vx >= width
    ) {
      return null;
    }
    if (
      cornersXYView[0].vy >= height &&
      cornersXYView[1].vy >= height &&
      cornersXYView[2].vy >= height &&
      cornersXYView[3].vy >= height
    ) {
      return null;
    }
  ...
  MOC.prototype.contains = function (ra, dec) {
    ...
    for (var curOrder = 0; curOrder <= this.order; curOrder++) {
      ipixMapByOrder[curOrder] = Math.floor(
        ipix / Math.pow(4, this.order - curOrder)
      );
    }
