...
export class HealpixIndex {
  ...
  ang2pix_nest(theta, phi) {
    ...
    if (Z0 >= za) {
      //Equatorial region
      const M = this.nside * (0.5 + tt);
      const y = this.nside * 0.75 * z;
      const u = M - y;
      const p = M + y;
      // o = u >>> this.order;
      // c = p >>> this.order;
  ...
  xyf2nest(ix, iy, face_num) {
    ...
    const nest =
      shiftLeft(face_num, 2 * this.order) +
      orAll(
        UTAB[255 & ix],
  ...
  nest2xyf(ipix) {
    ...
    let n = orAll(
      shiftRight(bigAnd(0x555500000000, i), 16),
  ...
  pix2ang_nest(ipix) {
    ...
    1 > jp && (jp += this.nl4);
    const phi = (jp - 0.5 * (kshift + 1)) * (Constants.PIOVER2 / nr);
    return { theta, phi };
  }
  ...
  pix2ang_ring(ipix) {
    ...
    if (this.ncap >= ipix1) {
      ...
      phi = ((iphi - 0.5) * Constants.PI) / (2 * iring);
    } else {
      if (this.npix - this.ncap > ipix) {
        ip = ipix - this.ncap;
        iring = ip / this.nl4 + this.nside;
        iphi = (ip % this.nl4) + 1;
        fodd = bigAnd(1, iring + this.nside) > 0 ? 1 : 0.5;
  ...
  integration_limits_in_costh(i_th) {
    ...
    if (this.nside >= i_th) {
      ...
      s =
        i_th === this.nside
          ? (2 * (this.nside - 1)) / 3 / a
          : 1 - Math.pow(i_th + 1, 2) / 3 / this.npface;
  ...
  pixel_boundaries(i_th, i_phi, i_zone, cos_theta) {
    ...
    if (1.5 * cos_theta >= 1) {
      ...
      kd = ku + 1;
      phi_l =
        Constants.PIOVER2 * (Math.max(ju * factor, 1 - kd * factor) + i_zone);
      phi_r =
        Constants.PIOVER2 * (Math.min(1 - ku * factor, jd * factor) + i_zone);
    } else if (1.5 * cos_theta > -1) {
      const cth34 = 0.5 * (1 - 1.5 * cos_theta);
      ...
      phi_l =
        Constants.PIOVER2 *
        (Math.max(cth34_1 - kd / r_n_nside, -cth34 + ju / r_n_nside) + i_zone);
      phi_r =
        Constants.PIOVER2 *
        (Math.min(cth34_1 - ku / r_n_nside, -cth34 + jd / r_n_nside) + i_zone);
    } else {
      sq3th = Math.sqrt(3 * (1 + cos_theta));
      factor = 1 / r_n_nside / sq3th;
      ...
      phi_l =
        Constants.PIOVER2 *
        (Math.max(1 - (M - ju) * factor, (M - kd) * factor) + i_zone);
      phi_r =
        Constants.PIOVER2 *
        (Math.min(1 - (M - jd) * factor, (M - ku) * factor) + i_zone);
    }
    return [phi_l, phi_r];
  }
  ...
  corners_ring(pix, step) {
    ...
    res[0] =
      l > p / 2 ? HealpixIndex.vector(M, g[1]) : HealpixIndex.vector(M, g[0]);
    g = this.pixel_boundaries(u, l, c, I[2]);
    res[f] =
      l > p / 2 ? HealpixIndex.vector(y, g[1]) : HealpixIndex.vector(y, g[0]);
    if (1 === step) {
  ...
  queryDisc(spatialVector, radius, nest, inclusive) {
    ...
    if (rlat2 >= Math.PI) {
      for (let m = h + 1; this.nl4 > m; ++m)
        this.inRing(m, 0, Math.PI, pixset, false);
    }
  ...
  ring2xyf(pix) {
    ...
    if (this.ncap > pix) {
      // North Polar cap
      iring = parseInt(0.5 * (1 + Math.sqrt(1 + 2 * pix)));
