...
const View = (function () {
    ...
    $(view.reticleCanvas).bind("click mouseout touchend", function (e) {
      ...
      if (e.type === "touchend" && view.pinchZoomParameters.isPinching) {
        ...
        view.pinchZoomParameters.initialFov =
          view.pinchZoomParameters.initialDistance = undefined;
  ...
  View.prototype.getVisibleCells = function (norder, frameSurvey) {
    ...
    for (var ipixIdx = 0, len = pixList.length; ipixIdx < len; ipixIdx++) {
      ...
      for (let k = 0; k < 4; k++) {
        if (corners) spVec.setXYZ(corners[k].x, corners[k].y, corners[k].z);

        // need for frame transformation ?
        if (frameSurvey && frameSurvey.system !== this.cooFrame.system) {
          if (frameSurvey.system === CooFrameEnum.SYSTEMS.J2000) {
            let radec = CooConversion.J2000ToGalactic([
              spVec.ra(),
              spVec.dec(),
            ]);
            lon = radec[0];
            ...
            let radec = CooConversion.GalacticToJ2000([
              spVec.ra(),
              spVec.dec(),
            ]);
            lon = radec[0];
  ...
  View.prototype.setZoom = function (fovDegrees) {
    if (
      fovDegrees < 0 ||
      (fovDegrees > 180 && !this.aladin.options.allowFullZoomout)
    ) {
      return;
    }
  ...
  View.prototype.setOverlayImageSurvey = function (
    ...
    if (typeof overlayImageSurvey == "string") {
      newOverlayImageSurvey =
        HpxImageSurvey.getSurveyFromId(overlayImageSurvey);
  ...
  View.prototype.setImageSurvey = function (imageSurvey, callback) {
    ...
    if (
      newImageSurvey &&
      this.imageSurvey &&
      Object.prototype.hasOwnProperty.call(newImageSurvey, "id") &&
      Object.prototype.hasOwnProperty.call(imageSurvey, "id") &&
